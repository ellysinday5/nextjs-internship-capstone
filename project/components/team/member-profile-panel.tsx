...
import { X, MessageSquare, ChevronDown, Search, Filter, Clock, Plus, Check } from "lucide-react"
import type { TeamMember } from "@/lib/team-data"
import { getMemberTasksAction, getMemberCommentsAction } from "@/actions/member-actions"
...
export function MemberProfilePanel({
  ...
  useEffect(() => {
    ...
    if (activeTab === "Task" && tasks.length === 0) {
      setLoading(true)
      getMemberTasksAction(member.userId, projectId).then(setTasks).finally(() => setLoading(false))
    }
    if (activeTab === "Comments" && comments.length === 0) {
      setLoading(true)
      getMemberCommentsAction(member.userId, projectId).then(setComments).finally(() => setLoading(false))
    }
  ...
  const saveMeta = (
    ...
    if (activityText) {
      ...
      newActivities = [{ id: crypto.randomUUID(), text: activityText, time: timeStr }, ...newActivities]
    }
    ...
    try { localStorage.setItem(key, JSON.stringify(newData)) } catch { /* ignore */ }
  }
              ...
              {dropdownOpen && (
                <div className="absolute left-0 top-16 z-50 w-52 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#14263e] shadow-xl overflow-hidden">
                  ...
                  {allMembers.map((m) => (
                    <button
                      key={m.id}
                      onClick={() => { onSelectMember(m); setDropdownOpen(false) }}
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {activeTab === "Activity" && (
            ...
            <>
              ...
              <div className="mb-5">
                <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-1.5">Time Off</p>
              ...
              {/* Project Manager */}
              <div className="mb-5">
                <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-1.5">Project Manager</p>
              ...
              <div className="mb-5 text-sm text-slate-500 dark:text-slate-400">
                <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-1.5">Email</p>
                <p>{displayEmail}</p>
              </div>
              ...
              <div className="mb-6">
                ...
                {isAddingPriority && (
                  <div className="mb-3 flex items-center gap-2">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && newPriority.trim()) {
                          saveMeta({ priorities: [...priorities, newPriority.trim()] }, `Added priority: ${newPriority.trim()}`)
                          setNewPriority("")
                    ...
                    <button onClick={() => setIsAddingPriority(false)} className="text-xs text-slate-400 hover:text-slate-600">
                      Cancel
                    </button>
                  </div>
                )}
                ...
                {priorities.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    {priorities.map((p, idx) => (
                      <div key={idx} className="flex items-center justify-between rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2.5 text-xs text-slate-700 dark:text-slate-300">
                        <span>{p}</span>
                        <button
                          onClick={() => {
              ...
              <div>
                ...
                {activities.length === 0 ? (
                  <p className="text-xs text-slate-400">Only the last 24 hours of activities will be shown here</p>
                ) : (
                  <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                    {activities.map((act) => (
                      <div key={act.id} className="flex justify-between rounded bg-slate-50 dark:bg-slate-800/20 p-2 text-xs text-slate-500 dark:text-slate-400">
                        <span>{act.text}</span>
          ...
          {activeTab === "Task" && (
            notLinked ? (
              <p className="text-xs text-slate-400">This member hasn't linked an account yet — no task data available.</p>
            ) : loading ? (
              ...
              <ul className="space-y-2">
                {tasks.map((t) => (
                  <li key={t.id} className="rounded-lg border border-slate-200 dark:border-slate-800 p-3 text-sm">
                    <p className="font-semibold text-slate-700 dark:text-slate-200">{t.title}</p>
                    <p className="text-xs text-slate-400">
                      {t.status}{t.dueDate ? ` · Due ${new Date(t.dueDate).toLocaleDateString()}` : ""}
                    </p>
                  </li>
                ))}
              </ul>
          ...
          {activeTab === "Comments" && (
            notLinked ? (
              ...
              <ul className="space-y-2">
                {comments.map((c) => (
                  <li key={c.id} className="rounded-lg border border-slate-200 dark:border-slate-800 p-3 text-sm">
